#!/usr/bin/env bash
# Put HTTPS in front of the Sentinel EB demo on a custom domain.
#
#   AWS_PROFILE=admin ./deploy/aws/enable-https.sh
#
# Requests (or reuses) an ACM cert for $DOMAIN, auto-creates the DNS validation
# record in Route 53, waits for ISSUED, then deploys a CloudFront distribution
# (TLS termination -> HTTP EB origin, WebSockets passed through) and a Route 53
# alias. Idempotent: safe to re-run.
set -euo pipefail

REGION="us-east-1"                 # CloudFront certs must live in us-east-1
DOMAIN="sentinel.sandip.dev"
ZONE_ID="Z09218533I2UQLLSE7RP4"    # sandip.dev hosted zone
STACK="sentinel-https"
...
if [ "$CERT_ARN" = "None" ] || [ -z "$CERT_ARN" ]; then
  ...
  # ACM needs a moment to populate the validation record.
...
aws cloudformation deploy \
  --stack-name "$STACK" \
  --template-file deploy/aws/sentinel-https.yaml \
  --region "$REGION" \
  --parameter-overrides \
