...
Last refreshed: 2026-07-21 19:07

Latest entry: [2026-07-21-1907-the-page-that-kept-no-record.md](entries/2026-07-21-1907-the-page-that-kept-no-record.md)

## Where we are now

**The Sentinel case study is live at sandip.dev/portfolio/Sentinel/, and the
walkthrough slot is filled** with `c7pvcOekoXk`. Description, architecture,
build versus buy, an inlined system diagram that re-themes with the palette
switcher. The source copy and the diagram live in this repo at
`docs/portfolio/`; the rendered page lives in the sandip.dev repo. The build
instructions in `docs/portfolio/CLAUDE.md` carry the three layout traps the page
took to get right, all of which are invisible in the finished result.

**There is now a record of who visits.** CloudFront standard logging writes to an
S3 bucket the `sentinel-https` stack owns, ninety-day lifecycle, cookies
excluded, retained on stack deletion. `deploy/aws/traffic-report.sh` reads it and
counts `/_stcore/stream` opens rather than requests, since one visit emits
hundreds of the latter. Before today the question was unanswerable: CloudWatch
gives request counts only, the nginx log dies with the instance, and the app's own
audit trail is wiped on every deploy and unreachable without SSM. The first eight
days of traffic are unrecoverable.

**Prod carries PR #37 through #41. Bundle `sentinel-20260721-012204.zip`, `main`
at `b810557`, EB Green. 674 tests, 2 skipped, ruff clean.** The Registry throws
nothing and its model-card popover produces a PDF, which is the thing that was
broken on the live site an hour ago.

**`app.py` is 157 lines and is the router.** It was 3,208: the stylesheet,
eleven screens, the chrome and the routing. The stylesheet is
`sentinel/ui/theme.py`, the chrome is `sentinel/ui/shell.py`, and every screen
is its own module under `sentinel/ui/screens/`. Deferred as a tidiness item
since 2026-07-17; it stopped being one when it became the reason ten worktrees
produced abandoned branches instead of parallel work.

**The split removed three cycles that the file itself had manufactured.** Three
renderers took a `nav_to` callback, each with a comment saying `app.py` imports
this module so importing back would be a cycle. All three were true and all
three existed only because one file owned both the callback and the import.
`nav_to` lives in `shell.py` now and the workaround is gone. One injection
survives and is the control on that claim: the audit screen imports govflow's
control popover, so that cycle is real.

**The deploy guard is written and has fired in anger once.** Two clauses: `HEAD`
is an ancestor of `origin/main`, and `git status --porcelain` is empty. The
tests slice the guard out of `deploy.sh` and run it in a throwaway repo, so they
exercise the shipped shell rather than a copy, and one case reproduces the
2026-07-20 near-miss exactly.

**Clicking through prod after the deploy found a `FileNotFoundError` that had
been live since roughly the first deploy.** The Registry writes its model-card
PDF under `runtime/`, which is gitignored and therefore absent from the bundle.
Every local checkout has one; prod never did. The existing test could not have
caught it, because it passed pytest's `tmp_path`, which pytest creates before
handing it over. **A fixture more generous than production is a fixture that
cannot fail.**
...
- [2026-07-21-1907-the-page-that-kept-no-record.md](entries/2026-07-21-1907-the-page-that-kept-no-record.md) : the portfolio case study shipped, the walkthrough is embedded, and then Sandip asked whether anyone had visited. **The record did not exist.** Three sources, all lossy: CloudWatch gives free request counts back to 13 July (881, 1146, 142, 22, 159, 379, 291, 2926, 141) which are requests and not people, since Streamlit emits hundreds per visit; Elastic Beanstalk hands back a hundred-line tail of the nginx log, about seven hours, in which one Airtel session opened `/_stcore/stream` beside nine of my own and the rest was the scanner background every public IP receives; and the app's own audit trail is gitignored, wiped on every deploy, and unreachable because the instance is not SSM-managed. **The build whose entire argument is that a run must be explicable afterwards kept no durable record of its own runs, and the surface making that argument was the one thing with no audit trail.** Fixed with CloudFront standard logging to an S3 bucket the stack owns, ninety-day lifecycle, cookies excluded, `DeletionPolicy: Retain`, plus the non-obvious half: legacy delivery uses a bucket ACL, so `BlockPublicAcls` must stay false or logging silently never writes. `traffic-report.sh` counts WebSocket opens rather than requests and prints the scanner noise as noise instead of discarding it; tested against a synthetic log with a known answer, since delivery lags an hour and the bucket was empty. **Instrumentation is not retroactive**: the first eight days are gone at any price, which is the argument for instrumenting a public surface the day it goes public rather than the day someone asks how it is doing. Also `docs/portfolio/CLAUDE.md`, build instructions carrying the three layout traps (the full-bleed maths, the `1fr` track whose `auto` floor held a 680px column open inside a 335px phone, the sticky rail painting over every breakout element), and an opaque background on the diagram. Prod untouched.
- [2026-07-21-0128-a-fixture-more-generous-than-prod.md](entries/2026-07-21-0128-a-fixture-more-generous-than-prod.md) — three carried-over follow-ups, and the last one found a bug that had been live for weeks. **The two-clause deploy guard**, three sessions late: `HEAD` an ancestor of `origin/main` AND a clean tree, with untracked files counting because a new module under `sentinel/` is inside the zip's file list. Tests slice the guard out of `deploy.sh` and run it in a throwaway repo, so they exercise the shipped shell; one case reproduces the near-miss exactly, and it printed `clean, and HEAD is on origin/main` on a real deploy an hour later. `stash@{0}` read and dropped. **`app.py` 3,208 lines to 157**, driven off the AST so comment blocks travelled with their code and with both CSS strings asserted byte-identical. The unexpected part: **three `nav_to` callback injections, each with a comment explaining a real cycle, all of which the file had manufactured by owning both the callback and the import.** Move `nav_to` to `shell.py` and there is nothing to work around. One injection survives and is the control on the claim, because the audit screen genuinely imports govflow. Also killed a module-level engine singleton that only worked because `app.py` re-executes every rerun, and a dead `ACCENT` duplicating a stylesheet token. Two ratchets: no `def` and no `<style>` in `app.py`, and every sidebar screen must have a dispatch branch. **Then the deploy, and clicking every screen on prod found `FileNotFoundError: 'runtime/model_card_...pdf'`** — gitignored, so absent from the bundle, present in every local checkout, broken on the live site since about the first deploy. **The existing test could not have caught it: it passed pytest's `tmp_path`, which pytest creates before handing it over.** A fixture more generous than production is a fixture that cannot fail. Fixed in `render_pdf`, checked in both directions. 674 tests.
...
- **A fixture more generous than production is a fixture that cannot fail.** Found 2026-07-21, clicking through prod after a deploy. `render_pdf` was tested with pytest's `tmp_path`, which pytest creates before handing it over, so the test asserted "renders a PDF given a directory that exists". The code needed to satisfy "renders a PDF", and the gap was invisible in the test's own text. On prod the directory is `runtime/`, gitignored and therefore not in the bundle, and the Registry threw `FileNotFoundError` for weeks while the suite stayed green. This is the third instance of the same shape on this build, after the `sqlglot` import crash behind a health 200 and the codegen allowlist granting five packages nothing installed. **The development environment is more permissive than prod, and every test runs in the permissive one.** The general fix is not more tests, it is fixtures that withhold what prod withholds: an empty directory tree, an absent optional dependency, a missing env var. Worth auditing every fixture in this suite for what it hands over for free.
- **A file large enough to own everything manufactures cycles that exist only because it owns everything.** Found 2026-07-21 splitting `app.py`. Three renderers took a `nav_to` callback, each carrying a comment that correctly explained an import cycle. Every one of those cycles existed because `app.py` owned `nav_to` and also imported the module that needed it. Moving `nav_to` into `shell.py` dissolved all three; the pattern had read like a design decision for months. The tell is a workaround whose justification names one specific file on both sides of the problem. The control on the claim is the injection that survived: govflow genuinely cannot import the audit screen, because the audit screen imports govflow.
...
- ~~Should `deploy.sh` refuse when `HEAD` is not an ancestor of `origin/main`?~~ **Resolved 2026-07-21: written, and the answer was two clauses rather than one.** The ancestor check alone would not have caught the near-miss it was designed for, because the near-miss checkout was on `main` at exactly `origin/main` with a revert staged. The second clause, an empty `git status --porcelain`, is the one that fires. Untracked files count, because a new module under `sentinel/` is inside the zip's file list and would ship having never been in git. Verifiable failure is fatal and inability to verify is a warning, matching the requirements guard beside it. Being behind main is allowed, since refusing it would block a rollback. The structural half of the question stands: the primary checkout still holds `main` and is still the only place the gitignored live-LLM key exists, so it cannot be abandoned for a worktree.
...
- Demo GIF/Loom for the README: dropped for now per Sandip (2026-07-14). Superseded 2026-07-21 by the published walkthrough, `c7pvcOekoXk`, embedded on the case study.
- Is anyone visiting the live app? Opened 2026-07-21 and only partly answerable. Standard logging is on now, so from today forward there is a per-request record and `traffic-report.sh` reads it. The window before today is not recoverable: one Airtel session in a seven-hour nginx tail is the entire evidence of the first eight days. Open sub-question: the sandip.dev case study is on GitHub Pages, which produces no server log at all, so referrers into the app are visible but visits to the page that refers them are not. A client-side counter is the only way to close that half, and nothing is installed.
